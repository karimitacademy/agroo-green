...
const Profiles = () => {
  ...
  if (typeof window !== "undefined") {
    ...
    const createProfile = async()=>{
        try{
        const resp = await axios.post('http://localhost:3000/profile',{...create})
    ...
    return (
        <div >
            ...
            {
   tt ? profile && (
        <div style={{background:'blue',borderRadius:"17px", width:"1300px",marginTop:"100px",color:"white",padding:"40px"}} key={profile.id}>
            <h1>Ваша карта</h1>
            <br />
            <br />
            <p style={{fontSize:'25px',marginBottom:'6px'}}>Имя : {profile.firstName}</p>
